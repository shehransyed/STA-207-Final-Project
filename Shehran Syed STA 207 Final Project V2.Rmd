...
for (ID in 1:5) {
  ...
  fr_neuron = matrix(nrow = n.neurons, ncol = n.trials)
  sfr_neuron = matrix(nrow = n.neurons, ncol = n.trials)
  fr_all = numeric(n.trials)
  sfr_all = numeric(n.trials)
  for (i in 1:n.trials) {
    fr_neuron[,i] = rowSums(session[[ID]]$spks[[i]])/0.4
    fr_all[i] = sum(fr_neuron[,i])/n.neurons
    
    sfr_neuron[,i] = (fr_neuron[,i] - min(fr_neuron[,i])) / (max(fr_neuron[,i]) - min(fr_neuron[,i]))
    sfr_all[i] = sum(sfr_neuron[,i])/n.neurons
  }
  ...
  s[[ID]] = list(
    ...
    trial_type = as.factor(paste0("L", session[[ID]]$contrast_left, "-R", session[[ID]]$contrast_right)),
    feedback_type = as.factor(session[[ID]]$feedback_type),
    fr_neuron = fr_neuron,
    sfr_neuron = sfr_neuron,
    fr_all = fr_all,
    sfr_all = sfr_all
  )
  ...
  #   contrast_left = as.factor(session[[ID]]$contrast_left),
  #   contrast_right = as.factor(session[[ID]]$contrast_right),
  #   trial_type = paste0("L", session[[ID]]$contrast_left, "-R", session[[ID]]$contrast_right),
  #   feedback_type = as.factor(session[[ID]]$feedback_type),
  #   fr_all = fr_all,
  #   sfr_all = sfr_all
  # )
...
```{r}
bp_fr_1 = ggplot(data = data, aes(session, fr_all)) +
  geom_boxplot(aes(fill = name))

bp_fr_2 = ggplot(data = data, aes(session, sfr_all)) +
  geom_boxplot(aes(fill = name))
...
```{r}
ggplot(data, aes(contrast_right, contrast_left, fill = sfr_all)) + 
  geom_tile() +
  facet_wrap(~session) +
